...
function serializeEventsSection(chart: ParsedChart): string[] {
	...
	// If the chart was originally parsed from .mid, unrecognizedEvents came in
	// as `[text]` (square-bracketed MIDI text meta events) — strip the brackets
	// so the .chart output writes the naked text between quotes (the .chart
	// E-event convention).
