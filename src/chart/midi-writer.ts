...
function buildEventsTrack(chart: ParsedChart): MidiEvent[] {
	...
	for (const ge of chart.unrecognizedEvents) {
		let text = ge.text
		if (!sourceIsMidi) {
	...
	// unrecognizedEvents. The parser splits [coda] into both places, but we
	// only need one.
	const hasCodaInGlobalEvents = chart.unrecognizedEvents.some(ge => {
...
function buildFretTrack(
	...
	// Use ENHANCED_OPENS mode only when any group contains both an open note
	// AND a non-open fret note (a chord-with-open). Otherwise use forceOpen
	// SysEx, which collapses chords with open into single open notes but
	// avoids conflicts with animations at easy diffStart.
	const useEnhancedOpens = trackDataEntries.some(td =>
		td.noteEventGroups.some(group => {
	...
	// Instrument-wide sections. Star power = 116, solo = 103 (no activation lanes on fret).
	for (const sp of deduplicateSections(allStarPower)) {
		const vel = (sp as { velocity?: number }).velocity ?? 100
		const ch = (sp as { channel?: number }).channel ?? 0
		addNoteOnOffWithChannel(events, sp.tick, sp.length, 116, vel, ch)
