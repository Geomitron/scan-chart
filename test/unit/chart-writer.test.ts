...
import { noteFlags, noteTypes, NoteEvent } from 'src/chart/types'
...
describe('writeChartFile round-trip: [Events]', () => {
	...
	it('preserves unrecognized global events', () => {
		...
		chart.unrecognized.eventsTrackTextEvents.push({ tick: 0, text: 'music_start', msTime: 0, msLength: 0 })
		const re = roundTrip(chart)
		expect(re.unrecognized.eventsTrackTextEvents.map(e => ({ tick: e.tick, text: e.text }))).toEqual([
	...
	it('does not duplicate an end event that also appears in unrecognizedEventsTrackTextEvents', () => {
		...
		chart.unrecognized.eventsTrackTextEvents.push({ tick: 1000, text: 'end', msTime: 0, msLength: 0 })
		const re = roundTrip(chart)
		expect(re.endEvents.map(e => e.tick)).toEqual([1000])
		expect(re.unrecognized.eventsTrackTextEvents.filter(e => e.text === 'end')).toHaveLength(0)
	})
})
...
describe('writeChartFile round-trip: unrecognized chart sections', () => {
	it('preserves unrecognized sections with arbitrary content', () => {
		const chart = createEmptyChart()
		chart.unrecognized.chartSections.push({
			name: 'MysteryBlock',
			lines: ['0 = foo', '100 = bar'],
		})
		expect(roundTrip(chart).unrecognized.chartSections).toEqual([
