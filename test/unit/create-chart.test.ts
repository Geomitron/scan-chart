...
import { defaultIniChartModifiers } from 'src/chart/types'
...
describe('createEmptyChart', () => {
	...
	it('produces a chart with empty metadata, tracks, and sections', () => {
		...
		expect(chart.unrecognized.eventsTrackTextEvents).toEqual([])
		expect(chart.unrecognized.eventsTrackMidiEvents).toEqual([])
		expect(chart.unrecognized.midiTracks).toEqual([])
		expect(chart.unrecognized.chartSections).toEqual([])
		expect(chart.unrecognized.syncTrackEvents).toEqual([])
