...
import { createEmptyChart } from 'src/chart/create-chart'
import { writeChartFile } from 'src/chart/chart-writer'
import { writeMidiFile } from 'src/chart/midi-writer'
import { parseChartAndIni, type ParsedChart } from 'src/chart/parse-chart-and-ini'
...
describe('writeChartFile round-trip: unrecognizedSyncTrackEvents', () => {
	it('preserves tempo anchors (A) verbatim', () => {
		const chart = createEmptyChart({ format: 'chart' })
		chart.unrecognized.syncTrackEvents.push(
			{ tick: 0, text: 'A 0' },
			{ tick: 3840, text: 'A 8805460' },
		)
		expect(roundTripChart(chart).unrecognized.syncTrackEvents).toEqual([
			{ tick: 0, text: 'A 0' },
			{ tick: 3840, text: 'A 8805460' },
		])
	})
	...
	it('preserves unknown / future SyncTrack event types verbatim', () => {
		const chart = createEmptyChart({ format: 'chart' })
		chart.unrecognized.syncTrackEvents.push({ tick: 1920, text: 'FUTURE 12 34 56' })
		expect(roundTripChart(chart).unrecognized.syncTrackEvents).toEqual([
	...
	it('does not disturb tempos or time signatures at the same tick', () => {
		...
		chart.unrecognized.syncTrackEvents.push({ tick: 960, text: 'A 12345' })
		const re = roundTripChart(chart)
		...
		expect(re.unrecognized.syncTrackEvents).toEqual([{ tick: 960, text: 'A 12345' }])
	})
	...
	it('writing to .mid drops anchors (no MIDI equivalent)', () => {
		...
		chart.unrecognized.syncTrackEvents.push({ tick: 1920, text: 'A 12345' })
		expect(roundTripMidi(chart).unrecognized.syncTrackEvents).toEqual([])
	})
})
...
describe('writeMidiFile round-trip: unrecognizedEventsTrackMidiEvents', () => {
	it('preserves Rock Band practice-mode assist-sample notes (24/25/26)', () => {
		...
		chart.unrecognized.eventsTrackMidiEvents.push(
		...
		const mapped = re.unrecognized.eventsTrackMidiEvents.map(e => ({
			tick: e.deltaTime,
			type: e.type,
	...
	it('is empty when no source MIDI events were on the EVENTS track', () => {
		const chart = createEmptyChart({ format: 'mid' })
		expect(roundTripMidi(chart).unrecognized.eventsTrackMidiEvents).toEqual([])
	})
	...
	it('writing to .chart drops MIDI EVENTS-track events (no .chart equivalent)', () => {
		...
		expect(roundTripChart(chart).unrecognized.eventsTrackMidiEvents).toEqual([])
	})
})
